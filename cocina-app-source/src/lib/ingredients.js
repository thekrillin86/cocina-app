...
export function parseIngredient(raw) {
  ...
  if (qtyMatch) {
    // Quita el "de" final: "600 g de" → "600 g"
    quantity = qtyMatch[1].replace(/\s+de\s*$/i, '').trim()
...
export function combineQuantities(a, b) {
  ...
  if (pa.value != null && pb.value != null) {
    ...
    // Misma unidad → sumar (conservando el plural original: "dientes", no "diente")
    if (pa.unit === pb.unit) {
      const total = pa.value + pb.value
      const unit =
        total > 1
          ? (pb.value > 1 ? pb.rawUnit : null) || (pa.value > 1 ? pa.rawUnit : null) || pa.rawUnit || pa.unit
          : pa.rawUnit || pa.unit
...
/* ============================================================
   ...
   Deduplica por nombre canónico, suma cantidades y guarda para
   cada producto en qué recetas y qué días se necesita.
   ============================================================ */
export function extractItemsFromMenu(menu, wid) {
  if (!menu || !menu.days) return []
  const seen = new Map()
  const weekKey = wid || menu.id || (menu.year && menu.week ? `${menu.year}-${String(menu.week).padStart(2, '0')}` : null)
  ...
  menu.days.forEach((d, dayIndex) => {
    ...
    for (const type of ['lunch', 'dinner']) {
      ...
      for (const raw of arr) {
        const subItems = String(raw).split(/,| y /i)
          ...
          if (!seen.has(key)) {
            ...
            if (ref.recipeName) {
              const dupe = existing.recipes.some(
                (r) =>
                  r.recipeName === ref.recipeName &&
                  r.dayIndex === ref.dayIndex &&
                  r.mealType === ref.mealType
              )
              if (!dupe) existing.recipes.push(ref)
            }
          }
        }
      }
  ...
  // Ordenar cada producto por el día en que primero se necesita
...
/* Primer día (índice 0-6) en que hace falta un producto */
export function firstNeededIndex(item) {
  if (!item.recipes || !item.recipes.length) return null
  return item.recipes.reduce(
    (min, r) => (r.dayIndex != null && r.dayIndex < min ? r.dayIndex : min),
    9
  )
}
